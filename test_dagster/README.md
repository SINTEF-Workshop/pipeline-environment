...
You can start writing assets in `test_dagster/assets/`. The assets are automatically loaded into the Dagster repository as you define them.
...
Tests are in the `test_dagster_tests` directory and you can run tests using `pytest`:

```bash
pytest test_dagster_tests
```
