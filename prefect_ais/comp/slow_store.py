...
from .nats_conn import NatsConn
...
class SlowStorage:
    def __init__(self):
        ...
        self.now = self.utc.localize(datetime.now())
    ...
    async def consumer(self, custom_func, durable_name):
        nc = await self.nats.get_connection("localhost:4222")
        psub = await self.new_subscriber(nc, durable_name)
