...
@task
def collect_messages(slow_storage):
    logger = get_run_logger()
    asyncio.run(slow_storage.consumer(slow_storage.append_to_dataframe, "ais"))
    logger.info(slow_storage.dataframe_list)
...
@flow(name="generate_trajectories")
def generate_trajectories():
    slow = SlowStorage()
    track_maker = TrackMaker()
    postgres = PostgresConn()
    
    messages = collect_messages(slow)
